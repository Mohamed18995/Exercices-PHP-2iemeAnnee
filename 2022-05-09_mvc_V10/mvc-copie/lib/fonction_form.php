...
function form($id, $action, $method, $content, $class = "", $with_upload = false){
    ...
    $form_enctype   = $with_upload ? " enctype=\"multipart/form-data\"" : "";
    ...
    foreach($content as $c){
        if(isset($c["html"])){
            ...
            if(isset($c["type"]) && $c["type"] == "file" && $with_upload){
                ...
                $missing_field_files = (!empty($superglobale) && empty($array_file_name)) ? true : false;

                $form      .= $c["input"].((!$required && $missing_field_files) ? "\t<p class=\"missingfield ".$class_msg."\"> ".$info_msg."</p>" : "");
                $form .= "\t</div>\n";
...
function addFilesInput($label, $properties, $required, $div_class = ""){
     ...
     *
     */
    return array("input" => $input, "check" => $check, "label" => $label, "error" => $error, "type" => "file");
}
...
function addInput($label, $properties, $required = false, $div_class = ""){
     ...
     *
     */
    return array("input" => $input, "check" => $check, "label" => $label, "error" => $error, "type" => $type);
}
